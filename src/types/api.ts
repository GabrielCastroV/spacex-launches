// To parse this data:
//
//   import { Convert, APISpaceXResponseQuery } from "./file";
//
//   const aPISpaceXResponseQuery = Convert.toAPISpaceXResponseQuery(json);
//
// These functions will throw an error if the JSON doesn't
// match the expected interface, even if the JSON is valid.

export interface APISpaceXResponseQuery {
    docs: Doc[];
    totalDocs: number;
    offset: number;
    limit: number;
...
export class Convert {
    public static toAPISpaceXResponseQuery(json: string): APISpaceXResponseQuery {
        return cast(JSON.parse(json), r("APISpaceXResponseQuery"));
    }

    public static aPISpaceXResponseQueryToJson(value: APISpaceXResponseQuery): string {
        return JSON.stringify(uncast(value, r("APISpaceXResponseQuery")), null, 2);
    }
}
